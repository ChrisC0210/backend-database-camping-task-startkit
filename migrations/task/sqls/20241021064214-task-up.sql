...
SET "role" = 'COACH'
WHERE
  email = 'starplatinum@hexschooltest.io' -- 1-3 刪除：刪除USER 資料表中，用 Email 找到透明人，並刪除該筆資料
DELETE FROM "USER"
WHERE
  email = 'opcatiy0@hexschooltest.io' -- 1-4 查詢：取得USER 資料表目前所有用戶數量（提示：使用count函式）
SELECT
  COUNT(*)
FROM
  "USER";
...
VALUES
  ('7堂組合包方案', 7, 1400),
  ('14堂組合包方案', 14, 2520),
  ('21堂組合包方案', 21, 4800) -- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
  -- 1. `王小明` 購買 `14 堂組合包方案`
  -- 2. `王小明` 購買 `21 堂組合包方案`
  -- 3. `好野人` 購買 `14 堂組合包方案`
  -- 1. `王小明` 購買 `14 堂組合包方案`
...
-- Query Result:
-- user_id	>>6954efd3-9f81-4e60-9c85-c46372146715
-- remaining_credit  >>33
-- ████████  █████   █     ███  
--   █ █   ██    █  █     █     
--   █ █████ ███ ███      ████  
...
-- 修改區間檢視練習內容資料 '2024-12-30 23:59:59'
